...
describe('Get users', () => {
        ...
        items: [
          {
            ...
            following_url: 'https://api.github.com/users/Babanila/following{/other_user}',
            gists_url: 'https://api.github.com/users/Babanila/gists{/gist_id}',
            starred_url: 'https://api.github.com/users/Babanila/starred{/owner}{/repo}',
            subscriptions_url: 'https://api.github.com/users/Babanila/subscriptions',
            organizations_url: 'https://api.github.com/users/Babanila/orgs',
            repos_url: 'https://api.github.com/users/Babanila/repos',
            events_url: 'https://api.github.com/users/Babanila/events{/privacy}',
            received_events_url: 'https://api.github.com/users/Babanila/received_events',
            type: 'User',
            site_admin: false,
            score: 1
          }
  ...
  describe('Handle multiple languages', () => {
    it('should call fetcher once with search endpoint response with data', async () => {
      ...
      const formatLanguages = jest.fn().mockReturnValue([`language:"javascript"`])
      const { getUsers } = createController(mockedGot, formatLanguages, baseUrl, timeout)
    ...
    it('should handle when no language is available on the users', async () => {
      ...
      const formatLanguages = jest.fn().mockReturnValue([`language:"java"`])
      const { getUsers } = createController(mockedGot, formatLanguages, baseUrl, timeout)
      const res = {
        end: function () {},
        status: function () {
    ...
    it('should call fetcher for fallback languages on empty response', async () => {
      const mockedGot = jest.fn().mockResolvedValueOnce(payload1).mockResolvedValueOnce(payload2)
      const formatLanguages = jest
        .fn()
    ...
    it('should handle no language in request', async () => {
      ...
      const formatLanguages = jest.fn().mockReturnValue([])
      const { getUsers } = createController(mockedGot, formatLanguages, baseUrl, timeout)
      const res = {
        end: function () {},
        status: function () {
  ...
  describe('Gets data', () => {
    ...
    it('should return valid object for found user', async () => {
      ...
      expect(results[0].name).toEqual('Babajide Williams')
      expect(results[0].followers).toEqual(1)
      expect(results[0].avatar_url).toEqual('https://avatars2.githubusercontent.com/u/23488237?v=4')
    })
